...
class TextModel(ModelBase):
    ...
    def _extract_pre_tokenizer_regexes(tokenizer_json: dict) -> list[str]:
        ...
        def adapt_for_cpp_regex(rx: str) -> str:
            # C++ std::regex (ECMAScript) does not support (?i:...) for
            # case-insensitive groups. Expand to explicit [cC] alternations.
            import re as _re
        ...
        def extract(step: dict) -> None:
            if step.get("type") == "Split" and "pattern" in step:
                ...
                for sub in step.get("pretokenizers", []):
                    extract(sub)
            # ByteLevel, Normalizer, etc. steps are handled by llama.cpp's
            # built-in byte_decode/normalizer logic — skip them here.
...
class LlamaModel(TextModel):
    ...
    def set_vocab(self):
        ...
        # MiniCPM5 uses the Llama3-style BPE tokenizer with ByteLevel decoder.
        # Keep this routing scoped to MiniCPM5 so other Llama-family models keep
        # their existing tokenizer selection behavior.
